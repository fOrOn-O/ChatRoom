...
describe('chat presentation helpers', () => {
  ...
  it('groups consecutive messages from the same author within five minutes', () => {
    const first = { from_id: 2, timestamp: morning }
    const close = { from_id: 2, timestamp: morning + 4 * 60 * 1000 }
    const late = { from_id: 2, timestamp: morning + 6 * 60 * 1000 }
  ...
  it('keeps cross-day messages separate', () => {
    const first = { from_id: 2, timestamp: morning }
    const nextDay = { from_id: 2, timestamp: morning + 24 * 60 * 60 * 1000 }
