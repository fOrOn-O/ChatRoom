...
- **并发连接管理**：每个连接独立读写，聊天消息经 Redis Streams 处理，Hub 以 Channel 管理连接生命周期并通过 RWMutex 保护在线状态。
...
`redis.key_prefix` 用于隔离不同应用或环境的键空间；`dial_timeout`、`read_timeout` 和 `write_timeout` 控制 Redis 网络操作超时。登录接口使用 Redis 同时限制来源 IP 和账号与 IP 组合的请求次数，默认窗口为 1 分钟；WebSocket 私聊和群聊按用户共享消息额度，默认 10 秒内允许 20 条。超限请求会收到明确错误，运行期间 Redis 检查异常时记录日志并继续原流程。聊天消息由 WebSocket 入口发布到 `redis.stream`，消费者完成 MySQL 持久化、实时投递和 ACK；处理失败的消息会进入 Pending 恢复流程，超过重试上限后转入死信 Stream。
